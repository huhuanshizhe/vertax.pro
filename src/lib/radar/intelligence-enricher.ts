...
import { validateEmailBatch, pickBestEmails, type EmailValidationResult } from './email-validator';
import { enrichLinkedIn, linkedInResultToContacts, type LinkedInEnrichmentResult } from './linkedin-enrich';
...
export async function enrichCandidateIntelligence(
  ...
  let linkedInPromise: Promise<LinkedInEnrichmentResult> | null = null;
  ...
  if (options?.includeContacts !== false) {
    ...
    // LinkedIn enrichment: Apollo People Search + 公司页面发现
    linkedInPromise = enrichLinkedIn(companyName, {
      domain,
    ...
    tasks.push(
      linkedInPromise
        .then((result) => {
          if (result.companyLinkedIn?.linkedInUrl || result.decisionMakers.length > 0) {
            console.log(
              `[RadarEnrich] LinkedIn found: ${result.decisionMakers.length} contacts, ` +
              `company page: ${result.companyLinkedIn?.linkedInUrl || 'none'}`
            );
          }
        })
        .catch((e) => errors.push(`LinkedIn enrich: ${e.message}`))
    );
  }
  ...
  // ==================== 邮箱验证管线 ====================
  if (contactSnapshot && contactSnapshot.emails.length > 0) {
    try {
      const allEmails = contactSnapshot.emails.map(e => e.value);
      const validation = await validateEmailBatch(allEmails, 5);
      console.log(
        `[RadarEnrich] Email validation: ${validation.summary.valid} valid, ` +
        `${validation.summary.invalid} invalid, ${validation.summary.risky} risky`
      );

      // 用验证结果更新 contactSnapshot 中邮箱的分数和状态
      const validationMap = new Map(validation.results.map(r => [r.email.toLowerCase(), r]));
      for (const email of contactSnapshot.emails) {
        const vResult = validationMap.get(email.value.toLowerCase());
        if (vResult) {
          email.confidence = vResult.score;
          if (!email.sources.includes('email_verified')) {
            email.sources = [...email.sources, 'email_verified'];
          }
        }
      }

      // 移除验证失败的邮箱（score < 30）
      const invalidEmails = validation.results
        .filter(r => !r.isValid && r.score < 30)
        .map(r => r.email.toLowerCase());
      if (invalidEmails.length > 0) {
        contactSnapshot.emails = contactSnapshot.emails.filter(
          e => !invalidEmails.includes(e.value.toLowerCase())
        );
      }
    } catch (error) {
      errors.push(`Email validation: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  // ==================== LinkedIn 结果合并 ====================
  const linkedInEnrichResult: LinkedInEnrichmentResult | null = linkedInPromise
    ? await linkedInPromise.catch(() => null)
    : null;
  if (linkedInEnrichResult) {
    // 合并决策者
    if (linkedInEnrichResult.decisionMakers.length > 0) {
      const liContacts = linkedInResultToContacts(linkedInEnrichResult);
      if (!intelligence.contacts) intelligence.contacts = {};
      const existingMakers = intelligence.contacts.decisionMakers || [];
      intelligence.contacts.decisionMakers = mergeDecisionMakers(
        existingMakers,
        liContacts.decisionMakers
      );
    }

    // 补充公司 LinkedIn URL
    if (linkedInEnrichResult.companyLinkedIn?.linkedInUrl) {
      if (!intelligence.contacts) intelligence.contacts = {};
      if (!intelligence.contacts.companyContacts) {
        intelligence.contacts.companyContacts = { emails: [], phones: [], linkedInUrls: [] };
      }
      const existingLiUrls = new Set(intelligence.contacts.companyContacts.linkedInUrls || []);
      if (!existingLiUrls.has(linkedInEnrichResult.companyLinkedIn.linkedInUrl)) {
        intelligence.contacts.companyContacts.linkedInUrls = [
          ...(intelligence.contacts.companyContacts.linkedInUrls || []),
          linkedInEnrichResult.companyLinkedIn.linkedInUrl,
        ];
      }
    }
  }
